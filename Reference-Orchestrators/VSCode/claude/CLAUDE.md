# FITPAC minimal loop (project instructions)

This project uses the **FITPAC minimal loop** for prose → spec → code → derived spec → comparison → reports. When the user describes what they want to build, run that loop.
...
When the project has a **src/** layout, code is still written under **app/** at project root.
...
2. **Execute** the sequence in that file: Socratic round (with **verbose** explanation of what you are doing and why) → write canonical spec → implement code under **app/** → derive spec from code → compare specs → write comparison report and plain-language run report. Use the user's message as the prose input.
3. **When a human is involved (prose-to-spec):** Be **verbose**. Explain why you are asking each question, how you are mapping their prose to the spec, and what defaults you are choosing and why.
